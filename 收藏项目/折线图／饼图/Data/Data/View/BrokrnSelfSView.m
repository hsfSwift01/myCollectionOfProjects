...
- (void)drawRect:(CGRect)rect
{
    ...
    //总高度-剩下的高度
    CGFloat floatY0 = self.AllHeight-[[self.pointArray objectAtIndex:0] intValue]*(self.AllHeight-100)/self.unityArray.count/self.everHeight;
    CGContextMoveToPoint(contextData1, 50, floatY0);//起始点
    ...
    for (int i = 0; i<self.pointArray.count; i++) {
        if (i !=0) {
            ...
            floatY0 = self.AllHeight-[[self.pointArray objectAtIndex:i] intValue]*(self.AllHeight-100)/self.unityArray.count/self.everHeight;
        }
    }
    CGContextSetFillColorWithColor(contextData1, [[UIColor greenColor]colorWithAlphaComponent:0.3].CGColor);//填充颜色
    CGContextDrawPath(contextData1, kCGPathFillStroke);//进行颜色填充
    ...
    //开始一个起始路径
    CGContextMoveToPoint(contextFull, 50,self.AllHeight);//起点
    CGContextAddLineToPoint(contextFull, 50, self.AllHeight-[[self.pointArray objectAtIndex:0] intValue]*(self.AllHeight-100)/self.unityArray.count/self.everHeight);
    CGContextAddLineToPoint(contextFull, 50+(self.pointArray.count-1)*(self.AllWidth+20)/self.pointArray.count, self.AllHeight-[[self.pointArray objectAtIndex:self.pointArray.count-1] intValue]*(self.AllHeight-100)/self.unityArray.count/self.everHeight);
    CGContextAddLineToPoint(contextFull, 50+(self.pointArray.count-1)*(self.AllWidth+20)/self.pointArray.count, self.AllHeight);
