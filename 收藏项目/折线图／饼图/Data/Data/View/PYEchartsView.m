...
/// Initialize
- (void)initAll {
    bundlePath = [[NSBundle mainBundle] pathForResource:@"Data" ofType:@"bundle"];
    ...
    if (bundlePath != nil) { // If 'iOS-Echarts' is installed by Cocoapods and don't use 'use_frameworks!' command
        ...
        if ([echartsBundle pathForResource:@"echarts" ofType:@"html"] == nil) {
            ...
            for (NSString *path in allFrameworks) {
                if ([path hasSuffix:@"Data.framework"]) { // if the framework name has suffix 'iOS_Echarts.framework', I think it's iOS-Echart's framework
                    bundlePath = [path stringByAppendingString:@"/Data.bundle"];
                    echartsBundle = [NSBundle bundleWithPath:bundlePath];
                    break;
                }
            }
